...
#define STK500V2_MAX_BODY_SIZE (275)
#define STK500V2_MIN_COMMAND_SIZE (5)
#define STK500V2_MAX_COMMAND_SIZE (STK500V2_MAX_BODY_SIZE + 6)
...
#define PARAM_SCK_DURATION                                                     \
  (0x98) /* SCK period index — used to configure SPI clock speed */
...
typedef struct {
  ...
  uint8_t Token;
  uint8_t MessageBody[STK500V2_MAX_BODY_SIZE];
  uint8_t Checksum;
...
typedef struct __packed {
  ...
  uint8_t CmdExecutionDelay; // Delay (in ms) in connection with the
                             // EnterProgMode command execution
  uint8_t SyncLoops;         // Number of synchronization loop
  uint8_t ByteDelay; // Delay (in ms) between each byte in the EnterProgMode
                     // command.
  uint8_t PollValue; // Poll value: 0x53 for AVR, 0x69 for AT89xx
  uint8_t PollIndex; // Start address, received byte: 0 = no polling, 3 = AVR, 4
                     // = AT89xx
...
typedef struct __packed {
  uint8_t CommandID;
  uint8_t EraseDelay; // Delay (in ms) to ensure that the erase of the device is
                      // finished
...
typedef struct __packed {
  uint8_t CommandID;
  uint8_t RetAddr; // Return address - indicates after which of the transmitted
                   // bytes on the SPI interface to store the return byte
  uint8_t Commands[4]; // Command bytes to be transmit
...
typedef struct {
  ...
  uint8_t Delay; // Delay, used for different types of programming termination,
...
typedef struct {
  ...
  uint8_t RxStartAddr; // Start address of returned data. Specifies on what
...
STK500V2_ParseCmd(ringbuf_t RingBuffer, STK500V2_CommandTypeDef *Stk500Command);
