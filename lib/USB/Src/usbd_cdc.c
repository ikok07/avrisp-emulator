...
__ALIGN_BEGIN static uint8_t
    USBD_CDC_CfgDesc[USB_CDC_CONFIG_DESC_SIZ] __ALIGN_END = {
        ...
        HIBYTE(CDC_DATA_FS_MAX_PACKET_SIZE), 0x00, /* bInterval */
...
static uint8_t USBD_CDC_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum) {
  ...
  if ((pdev->ep_in[epnum & 0xFU].total_length > 0U) &&
    ...
    if (((USBD_CDC_ItfTypeDef *)pdev->pUserData[pdev->classId])->TransmitCplt !=
        NULL) {
      ((USBD_CDC_ItfTypeDef *)pdev->pUserData[pdev->classId])
...
uint8_t USBD_CDC_SetTxBuffer(USBD_HandleTypeDef *pdev, uint8_t *pbuff,
  ...
  USBD_CDC_HandleTypeDef *hcdc =
      (USBD_CDC_HandleTypeDef *)pdev->pClassDataCmsit[ClassId];
#else
uint8_t USBD_CDC_SetTxBuffer(USBD_HandleTypeDef *pdev, uint8_t *pbuff,
                             uint32_t length) {
  USBD_CDC_HandleTypeDef *hcdc =
