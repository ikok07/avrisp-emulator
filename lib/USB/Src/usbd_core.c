...
USBD_StatusTypeDef USBD_RegisterClass(USBD_HandleTypeDef *pdev,
  ...
  if (pdev->pClass[pdev->classId]->GetHSConfigDescriptor != NULL) {
    pdev->pConfDesc =
        (void *)pdev->pClass[pdev->classId]->GetHSConfigDescriptor(&len);
  }
 ...
 */
USBD_StatusTypeDef
USBD_RegisterClassComposite(USBD_HandleTypeDef *pdev, USBD_ClassTypeDef *pclass,
  ...
  for (idx1 = 0; idx1 < pdev->NumClasses; idx1++) {
    ...
    if (pdev->tclasslist[idx1].Active == 1U) {
      ...
      if (pdev->pClass[pdev->classId] != NULL) {
        /* Free Class Resources */
        if (pdev->pClass[pdev->classId]->DeInit(
                pdev, (uint8_t)pdev->dev_config) != 0U) {
#if (USBD_DEBUG_LEVEL > 1U)
          USBD_ErrLog(
              "Class DeInit didn't succeed!, can't unregister selected class");
#endif /* (USBD_DEBUG_LEVEL > 1U) */
      ...
      /* Free the class location in classes table and reset its parameters to
       * zero */
 ...
 */
USBD_StatusTypeDef
USBD_RegisterDevStateCallback(USBD_HandleTypeDef *pdev,
                              USBD_DevStateCallbackTypeDef pUserCallback) {
  ...
  if (epnum == 0U) {
    ...
    if (pdev->ep0_state == USBD_EP0_DATA_OUT) {
      if (pep->rem_length > pep->maxpacket) {
        ...
        (void)USBD_CtlContinueRx(pdev, pep->pbuffer,
                                 MAX(pep->rem_length, pep->maxpacket));
      } else {
        ...
        if (idx < USBD_MAX_SUPPORTED_CLASS) {
          /* Setup the class ID and route the request to the relative class
           * function */
    ...
    if ((pdev->tclasslist[i].Active) == 1U) {
      if (pdev->pClass[i] != NULL) {
        ...
        if (pdev->pClass[i]->DeInit != NULL) {
          if (pdev->pClass[i]->DeInit(pdev, (uint8_t)pdev->dev_config) !=
              USBD_OK) {
            ret = USBD_FAIL;
          }
        }
 ...
 * @param  ep_type: USBD_EP_TYPE_CTRL, USBD_EP_TYPE_ISOC, USBD_EP_TYPE_BULK or
 * USBD_EP_TYPE_INTR
 * @param  ClassId: The Class ID
 * @retval Address of the selected endpoint or 0xFFU if no endpoint found.
 */
uint8_t USBD_CoreGetEPAdd(USBD_HandleTypeDef *pdev, uint8_t ep_dir,
                          uint8_t ep_type, uint8_t ClassId) {
  uint8_t idx;
