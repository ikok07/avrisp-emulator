...
size_t ringbuf_buffer_size(const struct ringbuf_t *rb) { return rb->size; }

void ringbuf_reset(ringbuf_t rb) { rb->head = rb->tail = rb->buf; }
...
const void *ringbuf_tail(const struct ringbuf_t *rb) { return rb->tail; }

const void *ringbuf_head(const struct ringbuf_t *rb) { return rb->head; }
...
size_t ringbuf_findchr(const struct ringbuf_t *rb, int c, size_t offset) {
  ...
  const uint8_t *start =
      rb->buf + (((rb->tail - rb->buf) + offset) % ringbuf_buffer_size(rb));
  assert(bufend > start);
...
void ringbuf_peek(ringbuf_t rb, uint8_t *dst, size_t count) {
  // save tail, copy, restore tail
  struct ringbuf_t tmp_view =
      *rb; // struct copy — only works if ringbuf_t is a pointer to struct
  ringbuf_memcpy_from(dst, &tmp_view, count);
}
